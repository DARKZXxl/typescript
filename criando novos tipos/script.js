...
const planetas = [];
let opcoes = '';
function planet(nome, coordenadas, situacao, satelites) {
    const obj = {
        nome,
        coordenadas,
        situacao,
...
function removeSatelit() {
    ...
    const index = planeta.satelites.findIndex((res) => res === sateliteName);
...
do {
    ...
    switch (opcoes) {
        case '1':
            ...
            const planetSituation = prompt('digite a situacao do planeta: habitado/habitavel/inabitavel');
            planet(name, coordenadas, planetSituation);
            break;
        case '2':
            atualizarSituacao();
            break;
